                                ...
                                <div class="col-md-8">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="">No. PTS</label>
                                            <input type="text" name="sco" id="sco" value="<?= $header['idPts'] ?>" class="form-control" readonly>
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            <label for="Tgl. PO">Tgl. PTS</label>
    ...
    $(document).ready(function() {
        ...
        if (a == '') {
            ...
            $(".completes").click(() => {
                ...
                $.ajax({
                    url: "<?= base_url('adminpo/completepts') ?>",
                    method: 'POST',
                    data: formx,
                    success: function(data) {
                        swal.fire({
                            title: 'PO Status Complete',
                            text: 'PTS tidak akan muncul ditable lagi',
                            icon: 'success'
                        }).then((rsk) => {
                            if (rsk.isConfirmed) {
                                window.location.href = "<?= base_url('adminpo/listpts') ?>";
                            }
                        });
                    }
                });
        ...
        $(".simpan").click(() => {
            swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "<?= base_url('adminpo/saveHeader_'); ?>",
                        method: 'post',
                        data: FormHeader,
                        success: function(data) {
                            console.log(data);
