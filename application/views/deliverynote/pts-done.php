...
<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h4 class="h4 mb-4 text-gray-800"><?= $title; ?></h4><small>Pick Ticket Sample - <em>Sudah di PO-XB</em></small>
    <div class="card mb-3 border-left-warning">
        <div class="card-body">
            <table class="table table-bordered" id="tb_pts" data-toggle="tb_pts" data-show-toggle="true" data-show-refresh="true" data-show-pagination-switch="true" data-show-columns="true" data-mobile-responsive="true" data-check-on-init="true" data-advanced-search="true" data-id-table="advancedTable" data-show-print="true" data-show-columns-toggle-all="true">
...
<script>
    $(document).ready(function() {
        ...
        $table.bootstrapTable({
            url: '<?= base_url('deliverynote/getPts'); ?>',
            pagination: true,
            search: true,
            fixedColumns: true,
            fixedNumber: 1,
            columns: [{
                ...
                formatter: function(value) {
                    if (lvl == '1' || lvl == '3') {
                        ...
                        return [
                            '<button class="btn btn-warning btn-sm lihat" data-pts="' + value + '" rel="tooltip" title="Lihat detail"><i class="fas fa-fw fa-eye"></i></button> ' +
                            '<button class="btn btn-primary btn-sm" rel="tooltip" title="Buatkan PO XB" disabled><i class="fas fa-fw fa-plus"></i> DN</button>'
                        ]
                    }
                }
            }]
        ...
        $('body').on('click', '#tb_pts .makepo', function() {
            var id = $(this).data('pts');
            window.location.href = "<?= base_url('deliverynote/makedns/') ?>" + id;
        });
    });
</script>